extends Node
# End-to-end play test for HARD on the six-button board: drives the REAL game.gd
# through synthetic taps at each button's on-screen position, so the whole path is
# exercised — tap -> HardGameUI.segment_at_point -> game.gd._player_pressed ->
...
#   Godot_..._console.exe --path . tools/hard_play_test.tscn      (needs a real GPU)
...
func _ready() -> void:
	...
	_ok(GameState.num_colors == 6, "hard is a six-colour game", str(GameState.num_colors))
	...
	_dev = _game._wheel as HardGameUI
	_ok(_dev != null, "hard plays on the six-button board")
	if _dev == null:
		get_tree().quit(1)
		return
	...
	if await _wait_for_input():
		# Tap all six in turn; only the first matters to the game, but each tap must
		# resolve to the button it landed on.
		for i in 6:
			var got := _dev.segment_at_point(_screen_of(i) - _dev.position)
			_ok(got == i, "a tap on button %d resolves to %d" % [i, i], "got %d" % got)

	print("\n-- a wrong press ends the game --")
	var seq: Array = _game.sequence.duplicate()
	var wrong: int = (int(seq[0]) + 3) % 6
