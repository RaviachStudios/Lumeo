...
const DIFFS := [
	{"diff": "easy", "title": "EASY", "accent": Color(0.28, 0.82, 0.45), "orb": 2,
		"icon": "leaf", "desc": "Perfect for beginners. Short sequences."},
	{"diff": "moderate", "title": "MODERATE", "accent": Color(1.00, 0.72, 0.25), "orb": 0,
		"icon": "chart", "desc": "A balanced challenge. Medium sequences."},
	{"diff": "hard", "title": "HARD", "accent": Color(0.95, 0.32, 0.40), "orb": 1,
		"icon": "flame", "desc": "For memory masters. Long sequences."},
]
...
func _make_card(d: Dictionary) -> Control:
	...
	mat.set_shader_parameter("radius", 28.0)
	...
	icon.position = Vector2(24, (CARD_H - dia) * 0.5)
	...
	btn.add_child(icon)
	var sym := _make_icon(d["icon"], dia * 0.34, accent.lightened(0.45))
	sym.position = Vector2(dia * 0.5, dia * 0.5)
	icon.add_child(sym)

	var tx := 24.0 + dia + 22.0
	var title := Label.new()
	title.text = d["title"]
	...
	title.position = Vector2(tx, 26)
	title.size = Vector2(CARD_W - tx - 60, 38)
	title.mouse_filter = Control.MOUSE_FILTER_IGNORE
	btn.add_child(title)
	...
	desc.add_theme_color_override("font_color", Color(0.74, 0.80, 1.0, 0.8))
	desc.position = Vector2(tx, 68)
	desc.size = Vector2(CARD_W - tx - 60, 38)
	desc.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	desc.mouse_filter = Control.MOUSE_FILTER_IGNORE
	btn.add_child(desc)

	# neon arrow in the accent color
	var arrow := Label.new()
	arrow.text = "›"
	...
	arrow.position = Vector2(CARD_W - 46, 0)
...
func _make_icon(kind: String, s: float, col: Color) -> Node2D:
	match kind:
		...
		"flame":
			return _icon_poly(_flame_poly(s), col)
		_:
...
func _flame_poly(s: float) -> PackedVector2Array:
	# teardrop: pointed tip at top, rounded body below
	var p := PackedVector2Array()
	p.append(Vector2(0, -s * 1.25))
	var n := 18
	var center := Vector2(0, s * 0.28)
	for i in n + 1:
		var a: float = lerp(-PI * 0.28, PI + PI * 0.28, float(i) / n)
		p.append(center + Vector2(cos(a), sin(a)) * s * 0.82)
	return p
...
func _build_bottom_card() -> void:
	var accent := Color(0.60, 0.45, 0.96)   # purple
	_bottom = Control.new()
	_bottom.custom_minimum_size = Vector2(CARD_W, 88)
	_bottom.size = Vector2(CARD_W, 88)
	...
	sh.code = CARD_SHADER
	mat.shader = sh
	mat.set_shader_parameter("rect_size", Vector2(CARD_W + CARD_PAD * 2.0, 88 + CARD_PAD * 2.0))
	mat.set_shader_parameter("pad", CARD_PAD)
	mat.set_shader_parameter("radius", 24.0)
	mat.set_shader_parameter("top_col", Vector3(0.10, 0.10, 0.30))
	mat.set_shader_parameter("bot_col", Vector3(0.07, 0.07, 0.22))
	mat.set_shader_parameter("rim_col", Vector3(accent.r, accent.g, accent.b))
	mat.set_shader_parameter("rim_boost", 0.8)
	var face := ColorRect.new()
	face.material = mat
	face.position = Vector2(-CARD_PAD, -CARD_PAD)
	face.size = Vector2(CARD_W + CARD_PAD * 2.0, 88 + CARD_PAD * 2.0)
	face.mouse_filter = Control.MOUSE_FILTER_IGNORE
	_bottom.add_child(face)

	var dia := 52.0
	var icon := Panel.new()
	icon.size = Vector2(dia, dia)
	icon.position = Vector2(22, (88 - dia) * 0.5)
	icon.mouse_filter = Control.MOUSE_FILTER_IGNORE
	var ic := StyleBoxFlat.new()
	var icbg := accent.darkened(0.2)
	icbg.a = 0.28
	...
	ic.shadow_size = 12
	icon.add_theme_stylebox_override("panel", ic)
	_bottom.add_child(icon)
	var sym := _icon_chart(dia * 0.32, accent.lightened(0.45))
	sym.position = Vector2(dia * 0.5, dia * 0.5)
	icon.add_child(sym)

	var tx := 22.0 + dia + 20.0
	var title := Label.new()
	title.text = "BEAT YOUR BEST"
	title.add_theme_font_size_override("font_size", 22)
	title.add_theme_color_override("font_color", Color(0.85, 0.82, 1.0))
	title.position = Vector2(tx, 16)
	title.size = Vector2(CARD_W - tx - 24, 28)
	title.mouse_filter = Control.MOUSE_FILTER_IGNORE
	_bottom.add_child(title)

	var desc := Label.new()
	desc.text = "Improve your memory and climb the leaderboards!"
	desc.add_theme_font_size_override("font_size", 15)
	desc.add_theme_color_override("font_color", Color(0.72, 0.78, 1.0, 0.8))
	desc.position = Vector2(tx, 46)
	desc.size = Vector2(CARD_W - tx - 24, 30)
	desc.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	desc.mouse_filter = Control.MOUSE_FILTER_IGNORE
	_bottom.add_child(desc)
...
func _layout() -> void:
	...
	var bottom_h := 88.0
	var total := DIFFS.size() * CARD_H + (DIFFS.size() - 1) * CARD_GAP + 24.0 + bottom_h
	var start_y := maxf(top + 150.0, sz.y * 0.54 - total * 0.5)
	for i in _cards.size():
		_cards[i].position = Vector2(cx - CARD_W * 0.5, start_y + i * (CARD_H + CARD_GAP))
	if _bottom:
		_bottom.position = Vector2(cx - CARD_W * 0.5,
			start_y + DIFFS.size() * (CARD_H + CARD_GAP) + 6.0)
...
func _intro() -> void:
	for n in [_header, _orbit, _back, _bottom]:
		if n:
			n.modulate.a = 0.0
